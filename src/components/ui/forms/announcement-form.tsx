...
import { announcementSchema, AnnouncementSchema } from "@/schemas/annoucement";
import { createAnnouncement } from "@/lib/api/annoucements";
...
export default function AnnouncementForm() {
  ...
  const [isCoverModalOpen, setIsImageModalOpen] = useState(false);
  ...
  const onSubmit = async (data: AnnouncementSchema) => {
    if (!data.image) {
      toast.error("Please upload the announcement image before submitting.");
      return;
    }
    ...
    try {
      ...
      if (result.statusCode !== 201) {
        const errorMsg =
          result.message || "Error when creating your announcement.";
        toast.error(errorMsg);
        return;
      }
      ...
      const errorMsg =
        ...
        error?.message ||
        "Unexpected error while creating announcement.";
      toast.error(errorMsg);
    } finally {
      setIsLoading(false);
    }
    ...
    <>
      <div className="max-w-[1100px] w-full">
        <h1 className="text-gray-800 flex gap-3 items-center mb-6">
          <TvMinimalIcon /> <p className="font-semibold">CREATE ANNOUNCEMENT</p>
        </h1>
